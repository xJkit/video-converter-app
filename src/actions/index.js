...
export const addVideos = videos => dispatch => {
  ...
  ipcRenderer.on('metadata:complete', (evt, data) => {
    dispatch({
      type: ADD_VIDEOS,
      payload: data,
    });
...
export const convertVideos = () => (dispatch, getState) => {
  const videos = Object.values(getState().videos);
  ...
  ipcRenderer.on('conversion:end', (evt, { video, FQPN }) => {
    dispatch({
      type: VIDEO_COMPLETE,
      payload: { ...video, outputPath: FQPN },
    });
  });
};
